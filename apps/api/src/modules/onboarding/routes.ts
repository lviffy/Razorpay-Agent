...
import { verifyShopifyCredentials, fetchShopifyProducts, syncShopifyToStore } from "../../integrations/shopify/index.ts";
...
Return ONLY valid JSON:
{
  ...
  "completionPercentage": number
      ...
      logger.warn({ err }, "[Backend Onboarding AI] Gemini call failed, using heuristic fallback");
    }
  }
...
router.post("/complete", async (req: Request, res: Response) => {
  try {
    ...
    const finalRzpKeyId = razorpayKeyId || env.RAZORPAY_KEY_ID || "";
    const finalRzpKeySecret = razorpayKeySecret || env.RAZORPAY_KEY_SECRET || "";
    const finalRzpWebhookSecret = razorpayWebhookSecret || env.RAZORPAY_WEBHOOK_SECRET || "";
    const finalWaPhone = whatsappPhoneNumber || phone || "+91 98765 00000";
    const finalWaPhoneId = whatsappPhoneNumberId || env.WHATSAPP_PHONE_NUMBER_ID || "";
    const finalWaToken = whatsappAccessToken || env.WHATSAPP_ACCESS_TOKEN || "";
    const finalWaVerify = whatsappWebhookVerifyToken || env.WHATSAPP_WEBHOOK_VERIFY_TOKEN || "zapai_meta_webhook_secret_2026";
    ...
    if (authHeader && authHeader.startsWith("Bearer ")) {
      try {
        ...
        logger.warn({ jwtErr }, "Could not extract user from token during onboarding completion");
      }
    }
...
// POST /api/v1/onboarding/sync-shopify
router.post("/sync-shopify", async (req: Request, res: Response) => {
  try {
    ...
    logger.error({ err }, "Onboarding sync-shopify error");
    return res.status(500).json({
