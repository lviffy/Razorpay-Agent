...
const GOOGLE_CALLBACK_URL = env.GOOGLE_CALLBACK_URL || `${APP_URL}/api/v1/auth/google/callback`;
  ...
  try {
    ...
    if (storeName && storeName.trim()) {
      try {
        ...
        logger.warn({ storeErr }, "Could not create dedicated store");
      }
    }
  ...
  try {
    ...
    if (tokens.id_token && GOOGLE_CLIENT_ID) {
      try {
        ...
        logger.warn({ idErr }, "verifyIdToken fallback to userinfo");
      }
    }
  ...
  try {
    ...
    if (credential && GOOGLE_CLIENT_ID) {
      try {
        ...
        logger.warn({ verifyErr }, "Google verifyIdToken error");
      }
    }
    ...
    if (code && GOOGLE_CLIENT_ID && GOOGLE_CLIENT_SECRET) {
      try {
        ...
        logger.warn({ codeErr }, "Google code exchange error");
      }
    }
    ...
    logger.error({ err }, "Google SSO error");
    return res.status(500).json({
...
router.get("/me", async (req: Request, res: Response) => {
  try {
    ...
    logger.error({ err }, "Fetch current user error");
